...
namespace quick_slot
{
    void get_configuration(Unknown* unknown)
    {
        ...
        g_pQuickSlot3->m_pos.x = x;
        g_pQuickSlot3->m_pos.y = y;
        ...
        CTexture::Create(&unknown->quickSlot2->image1, "data/interface", "main_slot_plus.tga", 32, 64);
    }
    ...
    void set_configuration(CQuickSlot* quickSlot)
    {
        ...
        if (quickSlot->id == 2)
        {
            auto x = std::to_string(g_pQuickSlot3->m_pos.x);
            auto y = std::to_string(g_pQuickSlot3->m_pos.y);
